...
def save_image(frame,  ep_num, step, folder_path ='output',invert=False):
    ...
    date_ep_string = str(currentDT.year) + str(currentDT.month) + str(currentDT.day)+ '_Episode_'+str(ep_num)
...
def convert_jpgs_to_vid(folder_path, ep_num):
    ...
    frame_dir = folder_path + str(currentDT.year) + str(currentDT.month) + str(currentDT.day)+ '_Episode_'+str(ep_num)
    os.chdir(frame_dir)

    # Save mp4
    frame_output = str(currentDT.year) + str(currentDT.month) + str(currentDT.day)+ '_Episode_'+str(ep_num) +  '.mp4'
    cmd_string = "ffmpeg -r 10  -i step%d.jpg -vcodec libx264 -crf 25  -pix_fmt yuv444p " +  frame_output
    os.system(cmd_string)

    # Save gif (converted from mp4)
    frame_gif_output =str(currentDT.year) + str(currentDT.month) + str(currentDT.day)+ '_Episode_'+str(ep_num) + '.gif'
    cmd_string = "ffmpeg  -t 10 -i " + frame_output + " -filter_complex \"[0:v] fps=12,scale=480:-1,split [a][b];[a] palettegen [p];[b][p] paletteuse\" " + frame_gif_output
    os.system(cmd_string)
