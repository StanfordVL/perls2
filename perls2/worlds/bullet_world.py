...
class BulletWorld(World):
    ...
    def __init__(self,
        ...
        # dictionary. Dictionary values are {<obj_name> : ObjectInterface}
        # e.g. {'013_apple' : BulletObjectInterface}
        for obj_idx, obj_key in enumerate(self.config['object']['object_dict']):
            obj_name = self.config['object']['object_dict'][obj_key]['name']
            pb_obj_id = self.arena.object_dict[obj_name]

            self.objects[obj_name] = BulletObjectInterface(
                config=self.config['object']['object_dict'][obj_key],
                physics_id=self._physics_id,
                obj_id=pb_obj_id)
    ...
    def reset(self):
        ...
        these steps.
        """
        logging.debug("Bullet World Reset")
        pass
